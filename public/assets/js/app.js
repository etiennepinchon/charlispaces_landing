...
let planeGeometry=new THREE.PlaneGeometry(2,4);let planeMaterial=new THREE.MeshBasicMaterial({color:0xffffff,map:bufferTexture.texture});let plane=this.texturePlane=new THREE.Mesh(planeGeometry,planeMaterial);plane.position.set(0,1.1,-4.01);plane.visible=false;document.querySelector('#mainScene').object3D.add(plane);rendererBuffer=new THREE.WebGLRenderer({antialias:false});rendererBuffer.setClearColor(0x000000);rendererBuffer.setPixelRatio(window.devicePixelRatio);rendererBuffer.setSize(resolution,resolution);rendererBuffer.domElement.style.zIndex=1000;document.body.appendChild(rendererBuffer.domElement);},createMesh(texture){var points=[];var i=0;var geometry=new THREE.Geometry();//if (this.tubeMesh) this.scene.remove(this.tubeMesh);
...
const API_URL='http://localhost:8080';window.store=[];const shakeEl=function(el){el.classList.add('shake');setTimeout(function(){el.classList.remove('shake');},300);};// Remove right click
...
const inputValue=this.modal.querySelector('.email').value;var data=new FormData();data.append("email",inputValue);fetch(`${API_URL}/newsletter`,{method:'POST',body:JSON.stringify({email:inputValue}),headers:{'content-type':'application/json'}}).then(response=>{wrapperEl.classList.remove('busy');if(response.status===200||response.status===201){wrapperEl.classList.add('hide-opacity');setTimeout(()=>{doneEl.classList.remove('hide');},300);setTimeout(()=>{doneEl.classList.add('show-opacity');patternEl.classList.add('show-opacity');},310);setTimeout(()=>{this.closeModalWithSuccess();},2200);const confetti=new _core_confetti__WEBPACK_IMPORTED_MODULE_9__["default"](document.body);setTimeout(()=>{confetti.stop();},2000);return;}return this.invalidEmail();}).catch(this.invalidEmail.bind(this));}}
